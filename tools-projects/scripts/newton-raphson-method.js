...
function calculate() {
    ...
    try {
        parsedFunction = math.parse(equation).compile();
        parsedDerivative = math.parse(derivative).compile();
    } catch (error) {
        alert('Error parsing function or derivative. Please check your input.');
        return;
    }

    const result = newtonsMethod(parsedFunction, parsedDerivative, initialGuess, maxIterations);
    displayResult(result);
    displaySteps(result.steps);
    updateGraph(result.steps, parsedFunction);
}
...
function newtonsMethod(parsedFunction, parsedDerivative, initialGuess, maxIterations) {
    const steps = [];
    let x = initialGuess;

    for (let i = 0; i < maxIterations; i++) {
        const fx = evaluateFunction(parsedFunction, x);
        const fPrimeX = evaluateFunction(parsedDerivative, x);
...
function evaluateFunction(parsedFunction, x) {
    try {
        return parsedFunction.evaluate({ x: x });
    } catch (error) {
...
// Graph update
function updateGraph(steps, parsedFunction) {
    ...
    const plotYValues = plotXValues.map(x => evaluateFunction(parsedFunction, x));
